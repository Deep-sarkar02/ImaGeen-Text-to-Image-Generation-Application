...
// here we will use the annimations of the motions
// in this componnet we will use the annimations
//  so we will impport the motiions here
...
const Header = () => {
    ...
    // now the onclick handler
    function onClickHandler() {
        // check the user is logged in or not so we will need the app context here
        // if the user is logged in then redirect to the result page
        if (user) {
            navigate("/results")
        }
        else {
    ...
    return (
        ...
        // now we can add the motion div here

        // motion.div is the usage of the framer motion
        // and add the eanimation 
        <motion.div className="flex flex-col justify-center items-center text-center my-20 " initial={{ opacity: 0.2, y: 100 }}
            transition={{ duration: 1 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}>

            {/* another dic on the annimation */}
            <motion.div className="text-stone-500 inline-flex text-center gap-2 bg-white px-6 py-1 rounded-full border border-nuetral-500"
            ...
            <motion.div className="flex flex-wrap justify-center mt-16 gap-3"
                ...
                {Array(6).fill('').map((item, index) => {
                    const sampleImages = [assets.sample_img_1, assets.sample_img_2, assets.sample_img_3, assets.sample_img_4, assets.sample_img_5, assets.sample_img_1];
                    return (
                        <motion.img
            ...
            <motion.p className='mt-2 text-neutral-600'
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ delay: 1.2, duration: 0.8 }}>Generated images from ImaGeen</motion.p>
        </motion.div>
        //  automatically this is the closing tag
    );
};
